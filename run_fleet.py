...
# Module-level tracker of the *current iter's* effective config snapshot.
# Populated at the top of each iter loop body so _write_md_sidecar can
# embed per-iter pipeline metadata (chains/frames/size/audio/tts) without
# having to thread the snapshot through every _write_sidecar call site.
# Falls back silently to {} when not set (e.g. tests / direct calls).
_CURRENT_ITER_CONFIG: dict = {}


def _is_handoff_frame(out_path: str) -> bool:
    """True when out_path is a chain-handoff frame (slop_<v>_<slug>_f<n>.png).
    These intermediate stills aren't user-facing artefacts and should not
    receive a markdown sidecar — only their parent FINAL/iter outputs do."""
    base = os.path.basename(out_path)
    return bool(re.search(r"_f\d+\.png$", base))


def _iter_components(out_path: str, v_idx) -> list:
    """Best-effort: list every slop_<v_idx>_<slug>_* file currently in
    OUTPUT_DIR that belongs to the same iter as out_path. Skips handoff
    frames and the .json/.md sidecars themselves so the components list
    only shows real outputs (base PNG, chain MP4s, music WAV, FINAL)."""
    try:
        base = os.path.basename(out_path)
        # Derive the iter's stem (slop_<idx>_<slug>) from the file's name.
        m = re.match(r"(slop_\d+_[^.]+?)(?:_[^_]+)?\.[a-z0-9]+$", base)
        stem = None
        if m:
            stem = m.group(1)
        if not stem and v_idx is not None:
            stem = f"slop_{v_idx}_"
        if not stem:
            return []
        out_dir = os.path.dirname(out_path) or "."
        # Glob both `slop_<idx>_<slug>*` and `FINAL_<idx>_<slug>*` so the
        # components list captures the silent FINAL alongside its audio twin.
        pats = [os.path.join(out_dir, f"{stem}*")]
        m2 = re.match(r"slop_(\d+)_(.+)", stem)
        if m2:
            pats.append(os.path.join(out_dir, f"FINAL_{m2.group(1)}_{m2.group(2)}*"))
        seen = set()
        names = []
        for pat in pats:
            for p in sorted(glob.glob(pat)):
                ...
                if n in seen:
                    continue
                seen.add(n)
                # Skip handoff frames + sidecars themselves.
                if _is_handoff_frame(n):
                    continue
                if n.endswith(".json") or n.endswith(".md"):
                    continue
                if n.endswith(".grid.json"):
                    continue
                names.append(n)
        return names
    except Exception:
...
def _write_md_sidecar(out_path, **fields):
    """Write a human-readable Markdown twin of the JSON sidecar so the
    prompt + provenance is browsable in any text editor / GitHub viewer /
    Obsidian without needing the slopfinity UI. Only emitted for
    FINAL/iter outputs (kind in {final, final-with-audio}) — chain parts
    and base images already have their JSON twins. Failure is non-fatal."""
    try:
        if _is_handoff_frame(out_path):
            return
        kind = fields.get("kind") or ""
        if kind not in ("final", "final-with-audio"):
            return
        snap = _CURRENT_ITER_CONFIG or {}
        prompt = fields.get("prompt") or ""
        model = fields.get("model") or "—"
        tier = fields.get("tier") or snap.get("tier") or "—"
        v_idx = fields.get("v_idx") or snap.get("_v_idx")
        ts = time.time()
        ts_iso = time.strftime("%Y-%m-%dT%H:%M:%S%z", time.localtime(ts)) or time.strftime("%Y-%m-%dT%H:%M:%S")
        base_model = snap.get("base_model", "—")
        video_model = snap.get("video_model") or snap.get("video_workflow") or "ltx-2.3"
        audio_model = snap.get("audio_model") or "none"
        tts_model = snap.get("tts_model") or "none"
        size = snap.get("size", "—")
        frames = snap.get("frames", "—")
        chains = fields.get("parts") or snap.get("chains") or "—"
        components = _iter_components(out_path, v_idx)
        components_md = "\n".join(f"- {c}" for c in components) if components else "- (none discovered)"
        md = (
            f"# {os.path.basename(out_path)}\n\n"
            f"**Prompt:** {prompt}\n"
            f"**Model:** {model}  •  **Kind:** {kind}  •  **Tier:** {tier}\n"
            f"**Generated:** {ts_iso}\n\n"
            f"## Pipeline\n"
            f"- Base image: {base_model}\n"
            f"- Video model: {video_model}\n"
            f"- Audio model: {audio_model}\n"
            f"- TTS: {tts_model}\n"
            f"- Size: {size}  •  Frames/part: {frames}  •  Chains: {chains}\n\n"
            f"## Components\n"
            f"{components_md}\n\n"
            f"---\n"
            f"*generated by slopfinity*\n"
        )
        side = out_path + ".md"
        with open(side, "w") as f:
            f.write(md)
    except Exception as e:
        print(f"   ⚠️  md sidecar write failed for {out_path}: {e}")
...
def _write_sidecar(out_path, **fields):
    ...
    Also emits a Markdown twin for FINAL/iter outputs via
    `_write_md_sidecar` so anyone can read prompt + provenance without the
    UI. The MD path is single-sourced through here — never call
    `_write_md_sidecar` directly from another stage."""
    try:
        side = out_path + ".json"
        with open(side, "w") as f:
    ...
    _write_md_sidecar(out_path, **fields)
...
def main():
    ...
    while True:
        ...
        # global config the fleet loaded at iter start. Stored at module
        # scope so _write_md_sidecar can read it without threading.
        global _CURRENT_ITER_CONFIG
