...
_STOPWORDS = {
    ...
    "but", "is", "are", "was", "were", "be", "been", "being", "as", "by", "it",
    "its", "this", "that", "these", "those", "from", "into", "over", "under",
    "very", "really", "just", "some", "any", "all",
}
...
def _resolve_ffmpeg_backend():
    ...
    try:
        subprocess.run(["docker", "exec", _DOCKER_FFMPEG_CTR, "ffmpeg", "-version"],
                       check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        _FFMPEG_BACKEND = "docker"
        print(f"[FLEET] ffmpeg backend: docker exec {_DOCKER_FFMPEG_CTR} (libopenh264 only)")
        return _FFMPEG_BACKEND
    except (FileNotFoundError, subprocess.CalledProcessError):
        pass
...
def update_state(mode="Idle", step="Waiting", video=0, total=0, chain=0, total_chains=0, prompt=""):
    ...
    while time.time() < deadline:
        try:
            ...
            with urllib.request.urlopen(req, timeout=5) as r:
                ...
                models = [m["id"] for m in data["data"] if "embed" not in m["id"].lower()]
                if models:
...
def generate_prompt(model_id, v_idx):
    ...
    pending_idx = next((i for i, x in enumerate(q) if x.get("status") in (None, "pending")), -1)
    if pending_idx >= 0:
        ...
        if model_id:
            try:
                ...
                print(f"[FLEET] LLM enhance failed for queue item: {e!r} — using raw prompt", flush=True)
    ...
    if config.get("infinity_mode"):
        ...
        try:
            ...
            req = urllib.request.Request("http://127.0.0.1:11434/v1/chat/completions", data=json.dumps(payload).encode("utf-8"), headers={"Content-Type": "application/json"})
            with urllib.request.urlopen(req, timeout=60) as response:
                content = json.loads(response.read())["choices"][0]["message"]["content"].strip()
...
def run_image_gen(model, prompt, out_path, tier="high"):
    ...
    print(f"🖼️  Image Gen [{model}] tier={tier} steps={qsteps} size={qsize} timeout={ito}s")
    ...
    if model == "qwen":
        ...
        cmd = [
            ...
            "-v", f"{qwen_out}:/root/.qwen-image-studio",
            "-w", "/workspace",
        ...
        if not pngs:
            raise Exception("Qwen generated no image (nothing in ~/.qwen-image-studio).")
        ...
        workflow = {
            "1": {"class_type": "LowVRAMCheckpointLoader", "inputs": {"ckpt_name": "ltx-2.3-22b-distilled-fp8.safetensors"}},
            "2": {"class_type": "LTXVGemmaCLIPModelLoader", "inputs": {"gemma_path": "gemma-3-12b-it-qat-q4_0-unquantized/model-00001-of-00005.safetensors", "ltxv_path": "ltx-2.3-22b-distilled-fp8.safetensors", "max_length": 1024}},
            "3": {"class_type": "CLIPTextEncode", "inputs": {"text": prompt, "clip": ["2", 0]}},
            "4": {"class_type": "CLIPTextEncode", "inputs": {"text": "blurry", "clip": ["2", 0]}},
            "5": {"class_type": "CFGGuider", "inputs": {"model": ["1", 0], "positive": ["3", 0], "negative": ["4", 0], "cfg": 1.0}},
            ...
            "9": {"class_type": "EmptyLTXVLatentVideo", "inputs": {"width": 1280, "height": 720, "length": 1, "batch_size": 1}},
            "10": {"class_type": "SamplerCustomAdvanced", "inputs": {"noise": ["8", 0], "guider": ["5", 0], "sampler": ["6", 0], "sigmas": ["7", 0], "latent_image": ["9", 0]}},
            "11": {"class_type": "LTXVSpatioTemporalTiledVAEDecode", "inputs": {"vae": ["1", 2], "latents": ["10", 0], "spatial_tiles": 4, "spatial_overlap": 1, "temporal_tile_length": 16, "temporal_overlap": 2, "last_frame_fix": False, "working_device": "auto", "working_dtype": "auto"}},
            "12": {"class_type": "SaveImage", "inputs": {"filename_prefix": f"ltx_base_{int(time.time())}", "images": ["11", 0]}}
        }
        try:
                ...
                if p_id in h:
                    ...
                    if 'images' in out:
                        ...
                        if len(imgs) == 1:
                            ...
                            patt = first.rsplit(num, 1)[0] + f"%0{len(num)}d" + first.rsplit(num, 1)[1]
                            print(f"   🎞️  Encoding {len(imgs)} frames...")
                            args = ["-y", "-framerate", "24", "-start_number", str(int(num)), "-i", f"comfy-outputs/{patt}", "-c:v", _ffmpeg_h264_encoder(), "-pix_fmt", "yuv420p", "-b:v", "8M", target_file]
                            _ffmpeg_run(args, check=True)
                            for f in imgs: 
                                try: os.remove(f"comfy-outputs/{f}")
                                except: pass
                            return True
    except Exception as e:
        if hasattr(e, 'read'): print(f"❌ ComfyUI Error Body: {e.read().decode('utf-8')}")
...
def generate_base_image_ltx23(prompt, output_path, size_str):
    ...
    workflow = {
        ...
        "2": {"class_type": "LTXVGemmaCLIPModelLoader", "inputs": {"gemma_path": "gemma-3-12b-it-qat-q4_0-unquantized/model-00001-of-00005.safetensors", "ltxv_path": "ltx-2.3-22b-distilled-fp8.safetensors", "max_length": 1024}},
        "3": {"class_type": "CLIPTextEncode", "inputs": {"text": prompt, "clip": ["2", 0]}},
        "4": {"class_type": "CLIPTextEncode", "inputs": {"text": "blurry, low quality", "clip": ["2", 0]}},
        "5": {"class_type": "CFGGuider", "inputs": {"model": ["1", 0], "positive": ["3", 0], "negative": ["4", 0], "cfg": 1.0}},
        "6": {"class_type": "KSamplerSelect", "inputs": {"sampler_name": "euler"}},
        "7": {"class_type": "BasicScheduler", "inputs": {"model": ["1", 0], "scheduler": "simple", "steps": 8, "denoise": 1.0}},
        "8": {"class_type": "RandomNoise", "inputs": {"noise_seed": seed}},
        "9": {"class_type": "EmptyLTXVLatentVideo", "inputs": {"width": w, "height": h, "length": 1, "batch_size": 1}},
        "10": {"class_type": "SamplerCustomAdvanced", "inputs": {"noise": ["8", 0], "guider": ["5", 0], "sampler": ["6", 0], "sigmas": ["7", 0], "latent_image": ["9", 0]}},
        "11": {"class_type": "LTXVSpatioTemporalTiledVAEDecode", "inputs": {"vae": ["1", 2], "latents": ["10", 0], "spatial_tiles": 4, "spatial_overlap": 1, "temporal_tile_length": 16, "temporal_overlap": 2, "last_frame_fix": False, "working_device": "auto", "working_dtype": "auto"}},
        "12": {"class_type": "SaveImage", "inputs": {"filename_prefix": f"base_{int(time.time())}", "images": ["11", 0]}}
    }
...
def generate_video_ltx(image_fn, prompt, out_path, size_str, frames):
    ...
    workflow = {
        ...
        "11": {"class_type": "LTXVImgToVideoConditionOnly", "inputs": {"vae": ["1", 2], "image": ["3", 0], "latent": ["10", 0], "strength": 1.0}},
        "12": {"class_type": "SamplerCustomAdvanced", "inputs": {"noise": ["9", 0], "guider": ["6", 0], "sampler": ["7", 0], "sigmas": ["8", 0], "latent_image": ["11", 0]}},
        "13": {"class_type": "LTXVSpatioTemporalTiledVAEDecode", "inputs": {"vae": ["1", 2], "latents": ["12", 0], "spatial_tiles": 4, "spatial_overlap": 1, "temporal_tile_length": 16, "temporal_overlap": 2, "last_frame_fix": False, "working_device": "auto", "working_dtype": "auto"}},
        ...
        while True:
            ...
            with urllib.request.urlopen(f"http://127.0.0.1:8188/history/{p_id}") as r:
                ...
                if p_id in h:
                    ...
                    out_rel = os.path.relpath(out_path, os.path.join(repo_root, "comfy-outputs"))
                    ...
                    args = [
                        ...
                        "-pix_fmt", "yuv420p", "-b:v", "8M",
                        out_path,
                    ]
                    _ffmpeg_run(args, check=True)
                    for f in imgs:
                        try: os.remove(os.path.join(repo_root, "comfy-outputs", f))
                        except: pass
                    return
...
def main():
    ...
    v_idx = 1
    matrix_mode = os.environ.get("FLEET_MATRIX", "").strip() in ("1", "true", "yes", "on")
    if matrix_mode:
        print("[FLEET] MATRIX MODE enabled — cycling through: " + ", ".join(
    ...
    try:
        ...
        if cleaned:
            cfg.save_queue(q0)
            print("[FLEET] swept stale `working` sentinels from previous run", flush=True)
    except Exception as e:
        print(f"[FLEET] startup working-sweep failed: {e!r}", flush=True)
    while True:
        config = cfg.load_config()
        if not config.get("infinity_mode") and v_idx > 1000: break
        ...
        if matrix_mode:
            ...
            print(f"[MATRIX] v{v_idx}: {b_mod} → {v_mod_forced} + audio={a_mod_forced}", flush=True)
        iter_failed = False
        try:
            ...
            if _task_opts.get("image_only") or _task_opts.get("skip_video"):
                update_state(mode="Completed", step="Image Only", video=v_idx, total=1000, prompt=p)
                print(f"[FLEET] image_only=True — skipping chain video stage", flush=True)
                v_idx += 1
                continue
            ...
            for c_idx in range(1, 11):
                ...
                if c_idx < 10:
                    ...
                    in_img = next_in
                    subprocess.run(["cp", next_in, f"{OUTPUT_DIR}/{_stem}_f{c_idx}.png"], check=True)

            update_state(mode="Finalizing", step="Final Merge", video=v_idx, total=1000, prompt=p)
            ...
            _ffmpeg_run(["-y", "-f", "concat", "-safe", "0", "-i", concat_path, "-c", "copy", final_silent], check=True)
            _write_sidecar(final_silent, prompt=p, model="ltx-2.3", kind="final", parts=len(chain_vids))
        ...
        try:
            if _task_opts.get("_seed_prompt"):
                ...
                eff_polymorphic = bool(live.get("polymorphic",
                                                live.get("chaos",
                                                         _task_opts.get("polymorphic", True))))
                eff_chaos = bool(live.get("chaos",
                                          live.get("polymorphic",
                                                   _task_opts.get("chaos", True))))
                eff_image_only = bool(live.get("image_only", _task_opts.get("image_only")))
                eff_skip_video = bool(live.get("skip_video", _task_opts.get("skip_video")))
                eff_when_idle = bool(live.get("when_idle", _task_opts.get("_when_idle", False)))
                eff_priority = live.get("priority", _task_opts.get("_priority", "next"))
                eff_config_snapshot = (live.get("config_snapshot")
                                       or _task_opts.get("_config_snapshot"))
                ...
                if eff_infinity and not cancelled_mid_flight:
                    if eff_polymorphic:
                        ...
                        rewritten = (_task_opts.get("_rewritten_prompt")
                    ...
                    print(f"[FLEET] ♾  re-queued infinity prompt ({tag}) to back of queue", flush=True)
                elif _task_opts.get("infinity") and cancelled_mid_flight:
                    print(f"[FLEET] ♾  infinity item cancelled mid-flight — NOT re-queueing", flush=True)
                elif _task_opts.get("infinity") and not eff_infinity:
                    print(f"[FLEET] ♾  user toggled infinity OFF mid-flight — NOT re-queueing", flush=True)
