...
# ROCm 6.0 Installation from AMD Repository Only
# Bypasses Ubuntu packages and uses only AMD official packages

set -e

echo "🚀 ROCm 6.0 Installation (AMD Repository Only)"
echo "=============================================="
...
echo ""
echo -e "${BLUE}🧹 Cleaning package cache and removing Ubuntu ROCm packages...${NC}"

# Remove Ubuntu ROCm packages specifically
sudo apt-get remove --purge -y \
    librocm-comgr2-2 \
    librccl2-2 \
    librocalution2-2 \
    libamd-comgr2-2 \
    libamd-comgr-dev || true
...
echo ""
echo -e "${BLUE}📦 Adding ONLY AMD official ROCm 6.0 repository...${NC}"

# Remove any existing ROCm repository files
sudo rm -f /etc/apt/sources.list.d/rocm.list
sudo rm -f /etc/apt/trusted.gpg.d/rocm.gpg

# Add ROCm 6.0 repository (using jammy for stability)
...
echo ""
echo -e "${BLUE}📦 Installing minimal ROCm 6.0 packages...${NC}"

# Install individual ROCm packages to avoid dependency conflicts
sudo apt-get install -y \
    rocminfo \
    rocm-smi

echo ""
echo -e "${BLUE}📦 Installing ROCm development packages...${NC}"

# Try installing dev packages individually
sudo apt-get install -y rocm-utils || echo "⚠️  Some packages may not be available"
...
echo ""
echo -e "${YELLOW}⚠️  If any packages failed to install, the basic ROCm functionality should still work.${NC}"