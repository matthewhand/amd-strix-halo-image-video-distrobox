...
# Master kill-switch for every automatic suggestion fetch path
# (page-load auto-suggest, GPU-idle prefetch, right-overlay edge fetch).
# When True the dashboard never calls /subjects/suggest on its own — the
# user must press the 🎲 Suggest button to populate the chip area. The
# manual button stays exempt by design.
