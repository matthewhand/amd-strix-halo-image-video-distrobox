...
def load_config():
    ...
    with Session(engine) as session:
        ...
        # Backward compat / First-time migration fallback. Runs BEFORE the
        # default-backfill so a stored config.json value wins over the hardcoded
        # default (otherwise every key is already filled and `k not in c` skips
        # the file — silently dropping the user's settings on first DB load).
        ...
        # Merge defaults for still-missing keys (last, so DB + file values win).
...
def _split_queue_item(item_dict):
    ...
    # prompt is a NOT NULL column; default missing prompts to "" so a minimal
    # item (e.g. a status-only row) saves instead of hitting an IntegrityError.
    item_dict.setdefault("prompt", "")
