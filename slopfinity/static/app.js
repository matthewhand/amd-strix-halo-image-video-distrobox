...
function connect() {
    ...
    ws.onmessage = e => {
        ...
        if (d.type === 'new_file') {
            ...
            // Parse model from filename. Conventions we support:
            //   test_<model>_N.png            → explicit test images
            //   v<N>_base.png                 → fleet base images (model inferred from config)
            //   v<N>_c<N>.mp4, v<N>_f<N>.png  → LTX chain outputs
            //   ltx_base_<ts>_00001_.png      → ComfyUI LTX base
            //   FINAL_<N>.mp4                 → concat output
            let model = '';
            const testMatch = file.match(/^test_([a-z0-9.-]+)_/i);
            if (testMatch) model = testMatch[1];
            else if (/^ltx_base_/i.test(file)) model = 'ltx-2.3';
            else if (/^v\d+_c\d+\.mp4$/i.test(file)) model = 'ltx-2.3';    // chain clips are always LTX
            else if (/^v\d+_f\d+\.png$/i.test(file)) model = 'ltx-bridge'; // frame bridge from chain
            const modelColors = {
                'qwen':       'badge-info',
                'ernie':      'badge-error',
                'ltx-2.3':    'badge-success',
                'ltx-bridge': 'badge-success',
                'wan2.2':     'badge-info',
                'wan2.5':     'badge-info',
                'heartmula':  'badge-secondary',
                'qwen-tts':   'badge-warning',
                'kokoro':     'badge-warning',
            };
            const mColor = modelColors[model] || 'badge-ghost';
            const c = document.createElement('div');
            const borderClass = isFinal ? 'border-accent' : (isV ? 'border-primary' : 'border-secondary');
            c.className = `card card-compact bg-base-100 shadow-lg border ${borderClass} card-hover overflow-hidden animate-pulse`;
            const kindLabel = isFinal ? '🎬 FINAL' : isV ? '⛓ chain' : '🖼 png';
            const kindClass = isFinal ? 'badge-accent' : isV ? 'badge-primary' : 'badge-secondary';
            const modelBadge = model
                ? `<span class="badge badge-xs ${mColor}" title="Generated by ${model}">${model}</span>`
                : '';
            const media = isV
                ? `<figure class="bg-black"><video controls autoplay muted loop class="w-full aspect-video object-cover"><source src="/files/${file}"></video></figure>`
                : `<figure class="bg-black"><img src="/files/${file}" class="w-full aspect-video object-cover" loading="lazy"></figure>`;
            c.innerHTML = `${media}
