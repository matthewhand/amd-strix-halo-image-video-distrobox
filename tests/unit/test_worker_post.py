...
def test_post_worker_comfy_path(tmp_path, monkeypatch):
    ...
    async def fake_comfy(self, in_p, out_p, prompt, **kwargs):
...
def test_post_worker_docker_mode_rejected(tmp_path, monkeypatch):
    ...
    assert "http" in (result.get("error") or "").lower()
